...
def model(input_shape):
    ...
    # 第一步：卷积层 (≈4 lines)
    # X = Conv1D(32, 15, padding='same')(X_input)  # CONV1D
    # X = BatchNormalization()(X)  # Batch normalization 批量标准化
    # X = Activation('relu')(X)  # ReLu activation ReLu 激活
    # X = Dropout(0.8)(X)  # dropout (use 0.8)

    # 第二步：第一个 GRU 层 (≈4 lines)
    X = GRU(units=64, return_sequences=True,kernel_regularizer=tfkreg.l2(lambda_l2),)(X_input)  # GRU (使用128个单元并返回序列)
...
model = tf.compat.v1.keras.models.load_model('./model/v5/my_model.h5')
model.summary()
opt = Adam(learning_rate=0.0006, beta_1=0.9, beta_2=0.999, decay=0.01)
model.compile(loss='binary_crossentropy', optimizer=opt, metrics=["accuracy"])
history = model.fit(xtrain,ytrain,batch_size=2,epochs=40,verbose=1,validation_split=0.1)
model.save('./model/v5/my_model.h5')
plt.figure()
...
plt.xlabel('epoch')
plt.ylabel('loss')
plt.savefig('image/log/v5/loss.jpg')
